// Vercel serverless function for POST /api/conversations
import { MongoClient, ServerApiVersion } from "mongodb";
import { GoogleGenAI } from "@google/genai";
import crypto from "node:crypto";
import process from "node:process";

// Cache the MongoDB client across invocations
let cachedClient = null;

async function getDb() {
  if (!cachedClient) {
    cachedClient = new MongoClient(process.env.DB_URI, {
      serverApi: {
        version: ServerApiVersion.v1,
        strict: true,
        deprecationErrors: true,
      },
    });
    await cachedClient.connect();
  }
  return cachedClient.db(process.env.DB_NAME);
}

export default async function handler(req, res) {
  if (req.method !== "POST") {
    res.status(405).json({ error: "Method not allowed" });
    return;
  }

  try {
    // Parse body for Vercel serverless
    const body = req.body || (await parseBody(req));
    const { constraint, context, output, persona, task } = body || {};
    if (!constraint || !context || !output || !persona || !task) {
      res.status(400).json({ error: "Missing required fields" });
      return;
    }

    // Generate ids
    const hash = crypto.randomUUID();
    const promptId = crypto.randomUUID();
    const createdAt = Date.now();
    const updatedAt = createdAt;

    // Call Gemini API
    const gemini = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
    const promptText =
      "Generate a response fully in markdown format (with multiple level headings when appropriate) for the following prompt built using the Pentagram Framework for prompt engineering. Do not mention the Pentragram Framework or that the response is being formatted as markdown: \n{" +
      `  constraint: ${constraint}\n` +
      `  context: ${context}\n` +
      `  format: ${output}\n` +
      `  persona: ${persona}\n` +
      `  task: ${task}\n` +
      `}`;
    const chat = gemini.chats.create({ model: "gemini-2.0-flash", history: [] });
    const geminiResponse = await chat.sendMessage({ message: promptText });
    const answer = geminiResponse.text;

    // Build prompt object
    const prompt = {
      id: promptId,
      constraint,
      context,
      format: output,
      persona,
      task,
      answer,
      createdAt,
      updatedAt,
    };

    // Save conversation to MongoDB
    const db = await getDb();
    const conversation = {
      hash,
      history: [prompt],
      createdAt,
      updatedAt,
    };
    await db.collection("conversations").insertOne(conversation);

    res.status(201).json(conversation);
  } catch (error) {
    console.error("API error:", error);
    res.status(500).json({ error: error.message || "Internal Server Error" });
  }
}

// Helper for Vercel body parsing
async function parseBody(req) {
  return new Promise((resolve, reject) => {
    let data = "";
    req.on("data", (chunk) => {
      data += chunk;
    });
    req.on("end", () => {
      try {
        resolve(JSON.parse(data));
      } catch (e) {
        reject(e);
      }
    });
  });
}
